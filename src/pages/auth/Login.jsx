...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (user) {
        // Redirect ke situs eksternal setelah login berhasil
        window.location.href = "https://sole-express.vercel.app/";
      } else {
    ...
    <div className="font-podkova min-h-screen flex items-center justify-center relative">
      ...
      <div className="bg-white rounded-2xl p-10 w-80 sm:w-96 flex flex-col items-center drop-shadow-lg mx-4">
        ...
        <form className="w-full mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="w-full bg-green-700 text-white text-xs font-semibold py-3 rounded-full hover:bg-green-800 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed"
            disabled={loading}
          >
            {loading ? (
